  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6 overflow-hidden relative">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 p-10 md:p-12 z-10"
      >
        <div className="text-center space-y-4 mb-12">
          ...
          <h1 className="text-3xl font-black tracking-tighter text-slate-900">LUMINA SYNC</h1>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Corporate Audit Enrollment</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-6">
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:border-slate-900 transition-all font-bold text-slate-800 text-sm placeholder:text-slate-200"
                  required
                />
              </div>
            </div>
            ...
            {/* Employee ID */}
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Corporate ID</label>
              <div className="relative group">
            ...
            {/* Access Tier */}
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Operations Role</label>
              <div className="flex gap-2 p-1.5 bg-slate-50 border border-slate-100 rounded-2xl">
                <button
                  type="button"
                  onClick={() => setRole('member')}
                  className={`flex-1 py-2.5 text-[9px] font-black uppercase tracking-widest transition-all rounded-xl ${role === 'member' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  Member
                </button>
                <button
                  ...
                  className={`flex-1 py-2.5 text-[9px] font-black uppercase tracking-widest transition-all rounded-xl ${role === 'lead' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  Lead
                </button>
              </div>
          ...
          <div className="pt-6 space-y-4">
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-5 bg-slate-900 text-white rounded-[1.25rem] shadow-xl shadow-slate-900/10 hover:bg-black transition-all font-black uppercase tracking-[0.2em] text-[10px] flex items-center justify-center gap-3 disabled:bg-slate-100 overflow-hidden relative group"
            >
