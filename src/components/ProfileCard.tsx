    ...
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden transition-colors h-full flex flex-col">
      <div className="p-8 border-b border-slate-100 bg-slate-50/50">
        <div className="flex items-center gap-5 mb-8">
          <div className="w-16 h-16 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-2xl font-black text-slate-200 shadow-sm relative">
            {profile.name[0]}
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <span className="px-2 py-0.5 bg-slate-900 text-white text-[8px] font-black uppercase tracking-widest rounded">{profile.role}</span>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-2 py-0.5 rounded">ID: {profile.employeeId}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm space-y-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Monthly Sync</p>
            <div className="flex items-end justify-between">
              <p className="text-xl font-black text-slate-900">{stats.monthly}%</p>
              <span className="text-[8px] font-black text-emerald-500 uppercase">Target</span>
            </div>
          </div>
          <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm space-y-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Yearly Goal</p>
            <p className="text-xl font-black text-slate-900">{stats.yearly}%</p>
          </div>
        </div>
      </div>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {profile.skills.map((skill) => (
              <div key={skill.name} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl group hover:border-blue-500/50 transition-all">
                <span className="text-[11px] font-bold text-slate-700">{skill.name}</span>
                <span className="w-5 h-5 bg-white border border-slate-200 rounded-md flex items-center justify-center text-[9px] font-black text-slate-400 group-hover:text-blue-500">
                  {skill.count}
                </span>
              </div>
            ))}
            {profile.skills.length === 0 && (
              <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">No competencies identified</p>
            )}
          </div>
        </section>

        <section className="p-6 bg-slate-900 rounded-2xl text-white shadow-xl shadow-slate-900/10 space-y-4">
